import { ConfigServiceConfig } from '../../../src/config/services';
...
describe('ConfigServiceConfig', () => {
  it('sets default value (enabled=true)', () => {
    const config = new ConfigServiceConfig();
    const mock = new MockContainer();

    config.applyEnvVarsTo(mock);

    expect(mock.envVars['FLOCI_SERVICES_CONFIGSERVICE_ENABLED']).toBe('true');
  });
  ...
  it('sets non-default value (enabled=false)', () => {
    ...
    expect(mock.envVars['FLOCI_SERVICES_CONFIGSERVICE_ENABLED']).toBe('false');
  });
